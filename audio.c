...
int main(int argc, char *argv[])
{
    ...
    const i64 frames_per_bar = audio.sample_count / BAR_COUNT;
    ...
    while (!WindowShouldClose())
    {
        ...
        {
            i64 start_frame = (audio.sample_count * i) / BAR_COUNT;
            i64 end_frame = (audio.sample_count * (i + 1)) / BAR_COUNT;
            i64 frame_count = end_frame - start_frame;

            if (frame_count <= 0)
                frame_count = frames_per_bar > 0 ? frames_per_bar : 1;
